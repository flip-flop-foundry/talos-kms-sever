...
class KMSServer {
    private static final Logger logger = LoggerFactory.getLogger(KMSServer.class)
    ...
    void start(KmsConfigBean kmsConfig) {
        ...
        try {
            ...
            if (ex.message.contains("File does not contain valid private key")) {
                logger.error("Failed to load private key. Is the password correct, or have you not provided a pkcs8 formatted key?")
            }
            throw ex
        }
        ...
        server = NettyServerBuilder.forAddress(new InetSocketAddress(kmsConfig.bindAddress, kmsConfig.port))
                ...
                .build()
                .start()
        logger.info("KMS Server started, listening on " + kmsConfig.bindAddress + ":" + kmsConfig.port)
    ...
    static void main(String[] args) {
        try {
            ...
            try {
                ...
                logger.error("Failed to load config file: ${e.message}", e)
            ...
            try {
                ...
                logger.error("Server failed to start or crashed: ${e.message}", e)
